...
function Input({ className, type, backgroundColor, icon, onClear, ...props }: Props) {
    return (
        <div className="group relative z-10 h-10 max-h-10 min-h-10 w-fit border border-neutral-600 bg-neutral-700/50 focus-within:bg-neutral-700/70 hover:bg-neutral-700/70">
            ...
            {onClear && (
                <button
                    className="absolute top-1/2 right-0 flex h-full w-9 -translate-y-1/2 cursor-pointer items-center justify-center text-white/50 hover:text-white focus-visible:text-white"
                    onClick={onClear}
                >
                    <X className="size-4" />
                </button>
